...
import (
	...
	apisCluster "github.com/exoscale/provider-exoscale-sos/apis/cluster"
	apisNamespaced "github.com/exoscale/provider-exoscale-sos/apis/namespaced"
	"github.com/exoscale/provider-exoscale-sos/config"
	"github.com/exoscale/provider-exoscale-sos/internal/clients"
	controllerCluster "github.com/exoscale/provider-exoscale-sos/internal/controller/cluster"
	controllerNamespaced "github.com/exoscale/provider-exoscale-sos/internal/controller/namespaced"
	"github.com/exoscale/provider-exoscale-sos/internal/features"
	"github.com/exoscale/provider-exoscale-sos/internal/version"
)
...
func main() {
	var (
		app                     = kingpin.New(filepath.Base(os.Args[0]), "Terraform based Crossplane provider for SOS").DefaultEnvars()
	...
	log := logging.NewLogrLogger(zl.WithName("provider-exoscale-sos"))
	if *debug {
	...
	mgr, err := ctrl.NewManager(cfg, ctrl.Options{
		...
		LeaderElectionID: "crossplane-leader-election-provider-exoscale-sos",
		Cache: cache.Options{
	...
	kingpin.FatalIfError(apisCluster.AddToScheme(mgr.GetScheme()), "Cannot add cluster-scoped SOS APIs to scheme")
	kingpin.FatalIfError(apisNamespaced.AddToScheme(mgr.GetScheme()), "Cannot add namespaced SOS APIs to scheme")
	...
	if canSafeStart {
		...
		kingpin.FatalIfError(controllerCluster.SetupGated(mgr, clusterOpts), "Cannot setup cluster-scoped SOS controllers")
		kingpin.FatalIfError(controllerNamespaced.SetupGated(mgr, namespacedOpts), "Cannot setup namespaced SOS controllers")
	} else {
		log.Info("Provider has missing RBAC permissions for watching CRDs, controller SafeStart capability will be disabled")
		kingpin.FatalIfError(controllerCluster.Setup(mgr, clusterOpts), "Cannot setup cluster-scoped SOS controllers")
		kingpin.FatalIfError(controllerNamespaced.Setup(mgr, namespacedOpts), "Cannot setup namespaced SOS controllers")
	}
